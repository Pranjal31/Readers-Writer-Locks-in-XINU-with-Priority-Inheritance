...
void lockwriter2 (char *msg, int lck)
{
	...
	for(i=0; i<32000; i++){
		// wasting CPU cycles - computation being simulated
	}
	for(i=0; i<32000; i++){
	       // wasting CPU cycles - computation being simulated
	}
...
void testlock ()
{
	...
	kprintf("\n\n***** Locks - Problem of priority inversion solved using Priority Inheritance *****\n\n");
        ...
        wr1 = create(lockwriter1, 2000, 90, "wr1", 2, "writer1", lck);
        wr2 = create(lockwriter2, 2000, 20, "wr2", 2, "writer2", lck);
        cpu1 = create(cpuboundproc, 2000, 50, "cpu1", 1, "cpu1");
        cpu2 = create(cpuboundproc, 2000, 60, "cpu2", 1, "cpu2");
	cpu3 = create(cpuboundproc, 2000, 70, "cpu3", 1, "cpu3");
	...
	kprintf("-start cpu1 (priority =  50)\n");
        resume (cpu1);
	
        kprintf("-start cpu2 (priority = 60)\n");
        resume (cpu2);
	
	kprintf("-start cpu3 (priority = 70)\n");
        resume (cpu3);
