...
SYSCALL releaseall (int numlocks, int args)
{
	...
	for(i=0; i< numlocks; i++){
		...
		if (isbadlock(lock) || (lptr= &locks[lock])->lstate==LFREE || !(lptr->lacquired[currpid]) ) {
			retcode = SYSERR;
		}
		else{	
			if(lptr->ltype == READ)
...
/*  
 *  Best reader (or writer) has highest prioriy among the waiting readers (or writers).
 *  Ties are broken using higher queue entry times. Since a new entry with same priority is inserted before an older entry, we just need to get the last reader (or writer).
 *  This reader (or writer) will have the highest priority as well as the highest wait time among other equal priority readers (or writers)
 */
